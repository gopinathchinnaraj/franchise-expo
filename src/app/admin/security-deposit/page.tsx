...
import securityDepositService from '@/lib/api/securityDeposit.service';
...
export default function AdminSecurityDepositPage() {
  ...
  const fetchTiers = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await securityDepositService.getAllDeposits();
      
      if (response.success && response.data) {
        const mappedTiers = response.data.map((item: any, index: number) => ({
          id: item.id,
          tierNo: index + 1,
      ...
      setError(error.message || 'Failed to load security deposits');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getCategoryFromBoothSq = (boothSq: string): "0-36" | "37-100" | "101+" => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        amountINR: formData.amountINR ?? 0,
        amountUSD: formData.amountUSD ?? 0,
        isActive: formData.isActive ?? true,
        displayOrder: formData.tierNo ?? 1
      };

      if (editingTier) {
        await securityDepositService.updateDeposit(editingTier.id, payload);
        setSuccessMessage('Tier updated successfully');
      } else {
        await securityDepositService.createDeposit(payload);
      ...
      setError(error.message || 'Failed to save security deposit');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      setError('');
      await securityDepositService.deleteDeposit(id);
      ...
      setError(error.message || 'Failed to delete security deposit');
    }
  };

  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      await securityDepositService.toggleActiveStatus(id, !currentStatus);
